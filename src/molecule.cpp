...
void Molecule::read(int flag)
{
  ...
  while (true) {
    ...
    try {
      ...
      if (nmatch != nwant) error->all(FLERR, "Invalid header line format in molecule file: {}", line);
    } catch (TokenizerException &e) {
      error->all(FLERR, "Invalid header in molecule file: {}", e.what());
    }
  }
  ...
  if (flag == 0) {
    ...
    if (bodyflag && !rmassflag)
      error->all(FLERR, "Molecule file needs Masses section for body particles");
